...
export function AttachmentForm({ initialData, courseId }: AttachmentFormProps) {
    ...
    return (
        <div className="mt-6 bg-slate-100 rounded-md p-4">
                    ...
                    {initialData.attachments.length > 0 && (
                        <div className="space-y-2">
                            {initialData.attachments.map((attachment) => (
                                <div key={attachment.id} className="flex itmes-center p-3 w-full bg-sky-100 border-sky-200 text-sky-700 rounded-md">
            ...
            {isEditing && (
                <div>
                    <FileUpload endpoint="courseAttachments" onChange={(url) => {
                        if (url) {
                            onSubmit({ url: url })
                        }
                    }} />
